...
file_rgx = re.compile(
    r"^(?:[^\t]+\t){1}([^\t]+)\t(?:en|und|ru|adm|\t)", re.IGNORECASE | re.UNICODE
)

# Bail early if we haven't been given a file to read
if original_file is None:
    print(
        "You must specify the full original file (usually hierarchy.txt or "
...
if new_filepath and os.path.exists(new_filepath):
    print(
        "This script writes {}, but that file already exists. Please move "
        "(or remove) that file and rerun this script.\n\nExiting.".format(new_filepath)
    )
...
for _type in ("region", "subregion", "city"):
    ...
    with open(filepath, "r") as f:
        ...
        for line in tqdm(
            f, total=num_lines, desc="Collecting geonameids from {}".format(filename)
        ):
            m = rgx.match(line)
...
with open(original_file, "r") as rf:
    ...
    if os.path.exists(new_filepath):
        print(
            "This script writes {}, but that file already exists. Please "
            "move (or remove) that file and rerun this script.".format(new_filepath)
        )
    ...
    with open(new_filepath, "a+") as wf:
        for line in tqdm(rf, total=num_lines, desc="Writing geonameids"):
